...
const soulGrowth = require('./src/modules/soul_growth');

// Register LLM caller for soul growth engine
soulGrowth.setLlmCall(async (opts) => {
  const { https } = require("https");
  return new Promise((resolve) => {
    try {
      const ep = opts.endpoint || "https://api.deepseek.com/v1/chat/completions";
      const key = opts.apiKey || "";
      const mdl = opts.model || "deepseek-chat";
      const url = new URL(ep);
      const options = {
        hostname: url.hostname, path: url.pathname, method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": "Bearer " + key, "User-Agent": "STELLAR-NYX/2.0" }
      };
      const req = https.request(options, (res) => {
        ...
        res.on("end", () => {
          try {
            if (res.statusCode >= 400) { resolve({ success: false, error: "API " + res.statusCode }); return; }
            const data = JSON.parse(body);
            if (data.error) { resolve({ success: false, error: data.error.message || JSON.stringify(data.error) }); return; }
      ...
      req.write(JSON.stringify({ model: mdl, messages: opts.messages, max_tokens: 2048, temperature: 0.5 }));
      req.end();
...
─
─
// IPC Handlers: Soul Growth
─

// llm:reflect -- lightweight LLM call (no persona injection)
ipcMain.handle("llm:reflect", async (event, { messages, model, apiKey, endpoint }) => {
  return new Promise((resolve) => {
    try {
      const ep = endpoint || "https://api.deepseek.com/v1/chat/completions";
      const key = apiKey || (loadConfig().llm || {}).apiKey || "";
      ...
      req.setTimeout(30000);
      req.write(JSON.stringify({ model: mdl, messages, max_tokens: 2048, temperature: 0.5 }));
      req.end();
    } catch (err) { resolve({ success: false, error: err.message }); }
  });
});

// soul:reflect -- reflect on conversation, append insights to growth log
ipcMain.handle("soul:reflect", async (event, { messages }) => {
  try {
    const appDataPath = app.getPath("userData");
    const cfg = loadConfig();
    const llmConfig = cfg.llm || {};
    if (!llmConfig.apiKey) return { success: false, error: "No LLM config" };
    const insights = await soulGrowth.reflectOnConversation(messages, llmConfig);
    if (insights.length === 0) return { success: false, error: "No insights extracted", count: 0 };
    soulGrowth.appendInsights(appDataPath, insights);
    const status = soulGrowth.getGrowthStatus(appDataPath);
    return { success: true, insights, insightCount: status.insightCount, readyToDistill: status.readyToDistill };
  } catch (err) { return { success: false, error: err.message }; }
});

// soul:distill -- distill growth log into SOUL.md
ipcMain.handle("soul:distill", async (event, { messages }) => {
  try {
    const appDataPath = app.getPath("userData");
    const cfg = loadConfig();
    const llmConfig = cfg.llm || {};
    if (!llmConfig.apiKey) return { success: false, error: "No LLM config" };
    const result = await soulGrowth.distillSoul(messages || [], llmConfig, appDataPath);
    if (result.success) {
      BrowserWindow.getAllWindows().forEach(win => {
        if (!win.isDestroyed()) win.webContents.send("soul:updated", { version: result.newVersion });
      });
    }
    return result;
  } catch (err) { return { success: false, error: err.message }; }
...
// soul:status -- get soul growth status
ipcMain.handle("soul:status", async () => {
  try { return soulGrowth.getGrowthStatus(app.getPath("userData")); }
  catch (err) { return { error: err.message }; }
});

// soul:log -- read growth log
ipcMain.handle("soul:log", async () => {
  try { return { success: true, log: soulGrowth.readGrowthLog(app.getPath("userData")) }; }
  catch (err) { return { success: false, error: err.message }; }
});

// soul:watch -- get auto-growth setting
ipcMain.handle("soul:watch", async () => {
  return { watch: loadConfig().soulWatch !== false };
});

// soul:setWatch -- enable/disable auto soul growth
ipcMain.handle("soul:setWatch", async (event, { enabled }) => {
  try {
    const cfg = loadConfig();
    cfg.soulWatch = enabled;
    fs.writeFileSync(CONFIG_FILE, JSON.stringify(cfg, null, 2), "utf-8");
    return { success: true, watch: enabled };
  } catch (err) { return { success: false, error: err.message }; }
});
...
const CONV_DIRconst CONV_DIR = path.join(app.getPath('userData'), '.anima', 'conversations');
...
ipcMain.handle('persona:loadDefault', async () => {
  try {
    const bundledPath = path.join(__dirname, 'src', 'assets', 'bundled-soul.tar.gz');
    if (!fs.existsSync(bundledPath)) {
      return { success: false, error: '默认人格包未找到', notFound: true };
    }
    
    const config = loadConfig();
    config.persona = {
      filePath: bundledPath,
      name: 'STELLAR NYX 2.0',
      loadedAt: new Date().toISOString()
    };
    fs.writeFileSync(CONFIG_FILE, JSON.stringify(config, null, 2), 'utf-8');
    
    return { success: true, name: 'STELLAR NYX 2.0', filePath: bundledPath };
  } catch (err) {
    return { success: false, error: err.message };
  }
});
